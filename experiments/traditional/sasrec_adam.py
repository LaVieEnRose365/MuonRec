...
# Ensure SASRecModules_ori.py is accessible in the same directory
try:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run supervised GRU/SASRec/Caser.")
    
    parser.add_argument('--epoch', type=int, default=500, help='Number of max epochs.')
    parser.add_argument('--data', nargs='?', default='Goodreads_5', help='Dataset name')
    ...
    # Default Learning Rate set to 1e-2 for Adam baseline
    ...
    # Default weight decay (l2_decay) set to 1e-6
...
def calculate_valid_loss(model, dataloader, device, loss_type, model_loss, item_num, args):
    """
    Calculate validation loss using the same negative sampling strategy (1 pos, 1 neg).
    """
    model.eval()
    total_loss = 0
    steps = 0
...
def main(topk, data_file_train, data_file_test, data_file_valid, data_dirs):
    if not args.debug:
        wandb.init(project="Rec", name=f"{args.data}_{args.model}", mode="disabled")
    else:
    ...
    # Initialize CosineAnnealingLR scheduler (T_max = args.epoch)
    ...
    print("Loading Validation Data (for Loss calculation)...")
    ...
    # Propensity score
    ps = calcu_propensity_score(train_df, item_num)
    ...
    for i in range(args.epoch):
        ...
        if (i + 1) % args.eval_num == 0:
            ...
            print(f"Epoch {i+1}: LR {current_lr:.6f} | Train Loss {avg_train_loss:.4f} | Valid Loss {current_valid_loss:.4f} | Valid NDCG {ndcg_last:.4f}")
